<?php
$datos_data_info_factura = "SELECT * FROM tbl15_info_asignar_grupo_a_grado WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual') AND (nombre_estado_factura = 'ABIERTA')";
$consulta_data_info_factura = mysqli_query($conectar, $datos_data_info_factura);
$data_info_factura = mysqli_fetch_assoc($consulta_data_info_factura);
$factura_ocupada = mysqli_num_rows($consulta_data_info_factura);

$cod_info_asignar_grupo_a_grado         = $data_info_factura['cod_info_asignar_grupo_a_grado'];
...
$cod_info_factura_strpad                = str_pad($cod_info_asignar_grupo_a_grado, 6, "0", STR_PAD_LEFT);
...
$(document).ready(function(){
    $("#cod_grado").on('change', function () {
            var valor = $(this).val();
            var campo = "cod_grado";
            var tipo_ajax = "tbl15_info_asignar_grupo_a_grado";
            $.post("guardar_info_factura_asignar_grupo_a_grado_temporal_ajax.php", { valor:valor, campo:campo, tipo_ajax:tipo_ajax, id:<?php echo $cod_info_asignar_grupo_a_grado; ?> }, function(data){
                $("#modelo").html(data);
        });
   });
});
...
<!-- //////////////////////////////////////////////////////////////////////////////////////////////////////////// -->
<form method="post" name="formulario" action="../admin/venta_asignar_grupo_a_grado_reg.php">
    <table class="table table-striped" border="1" cellspacing="0" cellpadding="20">
      <tr>
        ...
        <th style="text-align:right;">Cantidad de Grupos:</th>
        <th style="text-align:left;"><input class="input-block-level" name="und_grupo" id="und_grupo" type="number" value="" min="1" size="2" step="any" required/></th>

        <th style="text-align:right;">Generar Grupos</th>
        <th style="text-align:left;"><input type="image" src="../imagenes/guardar.png" name="vender" value="Guardar" /></th>
      </tr>
        <?php $pagina ='facturacion_asignar_grupo_a_grado.php'; ?>
        <input type="hidden" name="cod_info_asignar_grupo_a_grado" value="<?php echo $cod_info_asignar_grupo_a_grado ?>" size="10">
