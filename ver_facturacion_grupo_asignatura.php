...
<?php $nombre_pagina = "Notas Por Asignaturas - Detalle"; ?>
<!-- 1******************************************************* MODULO SUPERIOR *********************************************** -->
...
<table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
    <tbody><tr>
        <td bgcolor="#fff" align="center"><strong><a href="../admin/lista_asignar_alumno_a_grupo.php?pagina=<?php echo $pagina ?>">Ver Asignaturas Por Grupos</a></strong></td>
    </tr></tbody>
</table>
<br>
<!-- ***************************************************************************************************************************** -->
<?php include_once('../admin/ver_info_asignar_grupo_asignatura.php'); ?>
<!-- ***************************************************************************************************************************** -->
<!-- ***************************************************************************************************************************** -->
...
GROUP BY cod_asignatura ORDER BY cod_asignar_alumno_a_grupo DESC";
...
$cod_asignatura                                  = $datos_asignar_alumno_a_grupo['cod_asignatura'];
$nombre_asignatura                               = $datos_asignar_alumno_a_grupo['nombre_asignatura'];
$cod_docente                                     = $datos_asignar_alumno_a_grupo['cod_docente'];
...
?>
	<tr id="tr<?php echo $cod_asignar_alumno_a_grupo;?>">
		<td style="text-align:center;"><a href="../admin/ver_facturacion_asignar_grupo_asignatura_alumno.php?cod_info_asignar_alumno_a_grupo=<?php echo $cod_info_asignar_alumno_a_grupo?>&cod_asignatura=<?php echo $cod_asignatura ?>&cod_info_asignar_alumno_a_grupo=<?php echo $cod_info_asignar_alumno_a_grupo ?>&tab=<?php echo $tab ?>&tipo=<?php echo $tipo ?>&campo=<?php echo $campo ?>&cuenta=<?php echo $cuenta ?>&cod_caja_virtual=<?php echo $cod_caja_virtual ?>&pagina=<?php echo $pagina_local ?>"><img src="../imagenes/ver.png" class="img-polaroid" title="Ver Notas" alt=""></a></td>
        <td style="text-align:left;" id="nombre_producto_<?php echo $incre;?>"><?php echo $nombre_asignatura ?></td>
        <td style="text-align:left">
            <select name="cod_docente" id="<?php echo $cod_asignar_alumno_a_grupo;?>" class="chosen-select" data-show-subtext="true" data-live-search="true" style="width: 400px;">
            <?php if (isset($cod_docente)) { echo "<option value='' >Selecione</option>";
            } else { echo  "<option value='' selected >Selecione</option>"; }
            $sql_consulta2 = "SELECT cod_docente, identificacion_docente, nombre1_docente, nombre2_docente, apellido1_docente, apellido2_docente FROM tbl15_docente";
...
<script>  
 $(document).ready(function(){  
  ...
  $('select[name="cod_docente"]').change(function(){ 
  var cod_docente = $(this).val();  
  let id = this.id;
    $.ajax({ url:"guardar_docente-asignatura_editable_ajax_reg.php", method:"GET", data:{valor:cod_docente, campo:"cod_docente", id:id }, success:function(data){ $('#result').html(data); }  
    });  
  });
 });  
 </script>