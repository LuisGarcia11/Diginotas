...
$sql_info_alumno_gra = "SELECT cod_info_asignar_alumno_a_grupo, anyo FROM tbl15_info_asignar_alumno_a_grupo WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual') AND (nombre_estado_factura = 'ABIERTA')";
...
$datos_factura = "SELECT cod_asignar_alumno_a_grupo_temporal FROM tbl15_asignar_alumno_a_grupo_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual')";
...
xmlhttp.open("POST","../admin/busqueda_inmediata_asignar_alumno_a_grupo_temporal_php.php",true);
xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
xmlhttp.send("buscar="+valor_buscar+"&cod_info_asignar_alumno_a_grupo="+cod_info_asignar_alumno_a_grupo+"&anyo="+anyo+"&buscar_por="+buscar_por+"&nombre_tipo_moneda="+nombre_tipo_moneda+"&nombre_tipo_factura="+nombre_tipo_factura+"&tipo_busqueda="+tipo_busqueda+"&cod_estado_vacuna="+cod_estado_vacuna+"&cuenta="+cuenta+"&cod_caja_virtual="+cod_caja_virtual+"&pagina="+pagina);
}
</script>
...
<table align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:mono; width:100%">
    <tbody><tr>
        <td bgcolor="#fff" align="center"><strong><a href="../admin/lista_info_factura_grupo.php">LISTA DE GRUPOS</a></strong></td>
    </tr></tbody>
</table>
...
<br>
<!-- ***************************************************************************************************************************** -->
<?php include_once('../admin/info_asignar_alumno_a_grupo.php'); ?>
<!-- ***************************************************************************************************************************** -->
<!-- ***************************************************************************************************************************** -->
...
<tbody>
<?php
$sql_asignar_alumno_a_grupo_temporal = "SELECT * FROM tbl15_asignar_alumno_a_grupo_temporal WHERE (cuenta = '$cuenta_actual') AND (cod_caja_virtual = '$cod_caja_virtual') 
ORDER BY cod_asignar_alumno_a_grupo_temporal DESC";
$consulta_asignar_alumno_a_grupo_temporal = mysqli_query($conectar, $sql_asignar_alumno_a_grupo_temporal);
while ($datos_asignar_alumno_a_grupo_temporal = mysqli_fetch_assoc($consulta_asignar_alumno_a_grupo_temporal)) {

$cod_asignar_alumno_a_grupo_temporal    = $datos_asignar_alumno_a_grupo_temporal['cod_asignar_alumno_a_grupo_temporal'];
$cod_grupo                              = $datos_asignar_alumno_a_grupo_temporal['cod_grupo'];
$cod_alumno                             = $datos_asignar_alumno_a_grupo_temporal['cod_alumno'];
...
$incre++;
?>
	<tr id="tr<?php echo $cod_asignar_alumno_a_grupo_temporal;?>">
		<?php if ($cod_estado_eliminar_caja_mesa_virtual == '1') { ?><td  style="text-align:center;"><a href="../admin/eliminar.php?llave=<?php echo $cod_asignar_alumno_a_grupo_temporal?>&tab=<?php echo $tab ?>&tipo=<?php echo $tipo ?>&campo=<?php echo $campo ?>&cuenta=<?php echo $cuenta ?>&cod_caja_virtual=<?php echo $cod_caja_virtual ?>&pagina=<?php echo $pagina_local ?>"><img src="../imagenes/btn_excluir.png" class="img-polaroid" alt=""></a></td><?php } ?>
        <td style="text-align:center;" id="nombre_producto_<?php echo $incre;?>"><?php echo $identificacion_alumno ?></td>
        <td style="text-align:center;" id="nombre_producto_<?php echo $incre;?>"><?php echo $nombre_tipo_identificacion ?></td>
        <td style="text-align:left;" id="nombre_producto_<?php echo $incre;?>"><?php echo $nombres_alumno ?></td>
        <td style="text-align:left;" id="nombre_producto_<?php echo $incre;?>"><?php echo $apellidos_alumno ?></td>
	</tr id="tr<?php echo $cod_asignar_alumno_a_grupo_temporal;?>">
<?php } ?>
</tbody>
</table>
